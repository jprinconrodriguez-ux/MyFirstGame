...
-- === OPTIONAL: Jokers system (Milestone 3.2) ===
local Jokers_ok, Jokers = pcall(require, "jokers")
if not Jokers_ok then Jokers = nil end
local S = {}
...
-- Joker helper (shared S)
local function updateSharedState()
  if not S then S = {} end
  S.drawCards = function(n) return drawN(can_draw(n)) end
  S.combat = S.combat or {}
  S.turn_phase = GS.phase
end
...
local function applyLoadedState(state)
  ...
  -- GS
  if state.jokers and Jokers then S.jokers = state.jokers end
...
local function restartGame()
  ...
  updateSharedState()
  if Jokers then Jokers.init(S, love.math); Jokers.start_turn(S) end
  hand = {}
  selected = {}
  lastResult = nil
  moveCount = 0
...
function love.keypressed(key)
  ...
  if key == "j" then
    if GS.phase == "WIN" then setStatus("Game won—press R to restart."); return end
    updateSharedState()
    if Jokers and Jokers.can_use(S) then
      local res = Jokers.use(S, 1, { phase = GS.phase })
      if res and res.msg then setStatus("Joker: "..res.msg) else setStatus("Joker used.") end
    else
      setStatus("No joker available or already used this turn.")
    end
  end
